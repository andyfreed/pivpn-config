...
- **Interface roles pinned by MAC (built-in = client LAN, USB dongle = uplink)** — With two ethernet NICs the roles must not be allowed to swap. The client-LAN shared connection (`netplan-eth0`, 192.168.5.1) is pinned to the **built-in NIC's MAC**; the uplink (`eth1-uplink`) binds the **USB adapter by interface name**. Why it matters: the shared profile originally had no device pinning, so when the flaky USB dongle *revived after a reboot*, NetworkManager bound the shared 192.168.5.1 profile to the **dongle** instead of the built-in port — the built-in port (with the PC on it) went to `disconnected`, the PC lost DHCP/internet, and the `-i eth0` kill-switch rules were pointing at the wrong wire (happened + fixed 2026-07). `setup.sh` now detects the built-in NIC as the non-USB ethernet and pins the shared connection to its MAC; on this Pi the pin lives in `/etc/netplan/90-NM-*.yaml` (`match: macaddress:`), so it survives reboots.
...
- **sshd loses a boot race and dies until restarted.** Because `listen.conf` pins `ListenAddress 192.168.5.1`, sshd can start before NetworkManager brings that eth0 address up and fails with "Cannot assign requested address," staying dead (Tailscale SSH still works, so it's easy to miss). Fixed by the `ssh-wait-network.conf` drop-in (`After=network-online.target` + `Restart=on-failure`); installed by `setup.sh`. If a reboot ever leaves client-LAN SSH dead, `sudo systemctl restart ssh` is the immediate fix.
